(function() {
  var Logger, close_response, config, fetch_url, get_meta_status_code, ip_allowed, logger, metaTags, mommy, next_instance_number, page, parse_config, run_server, strip_scripts, system, url, webserver;
  ...
  fetch_url = function(url, response, this_inst, parse_delay, request_headers) {
    var done, evaluate_page, final_url, had_js_errors, headers, page_inst;
    final_url = url;
    headers = {};
    had_js_errors = false;
    done = false;
    ...
    page_inst.onConsoleMessage = function(msg, lineNum, sourceId) {
      if (config.log_messages) {
        return logger.info(this_inst, "[phantom:console] " + msg);
      }
    };
    page_inst.onCallback = function(data) {
      logger.info(this_inst, "[phantom:callback] Rendering: " + (data != null ? data.rendering : void 0));
      if (data != null ? data.rendering : void 0) {
    ...
    evaluate_page = function() {
      var statusCodeFromMeta;
      ...
      response.statusCode = statusCodeFromMeta || 200;
    ...
    return page_inst.open(url, function(status) {
      ...
      if (status !== "success") {
        ...
        return page_inst.parse_wait = setTimeout(evaluate_page, parse_delay);
      }
    });
  };
  ...
  get_meta_status_code = function(content) {
    return metaTags.parseStatusCode(content);
  };
