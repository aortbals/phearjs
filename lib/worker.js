(function() {
  var Logger, close_response, config, evaluate_page, fetch_url, get_meta_status_code, ip_allowed, logger, metaTags, mommy, next_instance_number, page, parse_config, run_server, strip_scripts, system, url, webserver;
    ...
    page_inst.onCallback = function(data) {
      ...
      if (data.rendering) {
        if (page_inst.parse_wait) {
          ...
          return evaluate_page(page_inst, response, page_inst.status);
        }
      }
    };
    return page_inst.open(url, function(status) {
      ...
      if (status !== "success") {
        ...
        return page_inst.parse_wait = setTimeout((function() {
          return evaluate_page(page_inst, response, status);
        }), parse_delay);
      }
    });
  };

  evaluate_page = function(page_inst, response, status) {
